...
build-all: ## Build go binary for all supported platforms
	...
	GOOS=linux GOARCH=amd64 GO_ENABLED=0 go build -o magecomm-linux-amd64 \
    && GOOS=linux GOARCH=arm64 GO_ENABLED=0 go build -o magecomm-linux-arm64 \
	&& GOOS=windows GOARCH=amd64 GO_ENABLED=0 go build -o magecomm-windows-amd64.exe \
	&& GOOS=darwin GOARCH=amd64 GO_ENABLED=0 go build -o magecomm-darwin-amd64 \
	&& GOOS=darwin GOARCH=arm64 GO_ENABLED=0 go build -o magecomm-darwin-arm64
